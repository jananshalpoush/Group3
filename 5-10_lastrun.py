...
expName = '5-10'  # from the Builder filename that created this script
...
def setupData(expInfo, dataDir=None):
    ...
    thisExp = data.ExperimentHandler(
        ...
        originPath='/Users/ohtalab2/Desktop/Experiment Folder/5-10_lastrun.py',
...
def setupDevices(expInfo, thisExp, win):
    ...
    if deviceManager.getDevice('groupSelection') is None:
        # initialise groupSelection
        groupSelection = deviceManager.addDevice(
            deviceClass='keyboard',
            deviceName='groupSelection',
        )
...
def run(expInfo, thisExp, win, globalClock=None, thisSession=None):
    ...
    GroupInstructions = visual.TextStim(win=win, name='GroupInstructions',
        ...
        depth=0.0);
    groupSelection = keyboard.Keyboard(deviceName='groupSelection')
    ...
    groupSelection.keys = []
    groupSelection.rt = []
    _groupSelection_allKeys = []
    # Run 'Begin Routine' code from groupSelectionCode
    selectedGroup = None
    # keep track of which components have finished
    GroupSelectionComponents = [GroupInstructions, groupSelection]
    ...
    while continueRoutine and routineTimer.getTime() < 5.0:
        ...
        # *groupSelection* updates
        waitOnFlip = False
        
        # if groupSelection is starting this frame...
        if groupSelection.status == NOT_STARTED and tThisFlip >= 0.0-frameTolerance:
            # keep track of start time/frame for later
            groupSelection.frameNStart = frameN  # exact frame index
            groupSelection.tStart = t  # local t and not account for scr refresh
            groupSelection.tStartRefresh = tThisFlipGlobal  # on global time
            win.timeOnFlip(groupSelection, 'tStartRefresh')  # time at next scr refresh
            # add timestamp to datafile
            thisExp.timestampOnFlip(win, 'groupSelection.started')
            # update status
            groupSelection.status = STARTED
            # keyboard checking is just starting
            waitOnFlip = True
            win.callOnFlip(groupSelection.clock.reset)  # t=0 on next screen flip
            win.callOnFlip(groupSelection.clearEvents, eventType='keyboard')  # clear events on next screen flip
        
        # if groupSelection is stopping this frame...
        if groupSelection.status == STARTED:
            # is it time to stop? (based on global clock, using actual start)
            if tThisFlipGlobal > groupSelection.tStartRefresh + 5-frameTolerance:
                # keep track of stop time/frame for later
                groupSelection.tStop = t  # not accounting for scr refresh
                groupSelection.tStopRefresh = tThisFlipGlobal  # on global time
                groupSelection.frameNStop = frameN  # exact frame index
                # add timestamp to datafile
                thisExp.timestampOnFlip(win, 'groupSelection.stopped')
                # update status
                groupSelection.status = FINISHED
                groupSelection.status = FINISHED
        if groupSelection.status == STARTED and not waitOnFlip:
            theseKeys = groupSelection.getKeys(keyList=['1','2','3'], ignoreKeys=["escape"], waitRelease=False)
            _groupSelection_allKeys.extend(theseKeys)
            if len(_groupSelection_allKeys):
                groupSelection.keys = _groupSelection_allKeys[-1].name  # just the last key pressed
                groupSelection.rt = _groupSelection_allKeys[-1].rt
                groupSelection.duration = _groupSelection_allKeys[-1].duration
                # a response ends the routine
                continueRoutine = False
        # Run 'Each Frame' code from groupSelectionCode
        if '1' in event.getKeys():
            selectedGroup = 1
            continueRoutine = False
        elif '2' in event.getKeys():
            selectedGroup = 2
            continueRoutine = False
        elif '3' in event.getKeys():
            selectedGroup = 3
            continueRoutine = False
    ...
    # check responses
    if groupSelection.keys in ['', [], None]:  # No response was made
        groupSelection.keys = None
    thisExp.addData('groupSelection.keys',groupSelection.keys)
    if groupSelection.keys != None:  # we had a response
        thisExp.addData('groupSelection.rt', groupSelection.rt)
        thisExp.addData('groupSelection.duration', groupSelection.duration)
    ...
    continueRoutine = True
    # update component parameters for each repeat
    thisExp.addData('Initialization.started', globalClock.getTime(format='float'))
    # keep track of which components have finished
    ...
    for thisComponent in InitializationComponents:
        if hasattr(thisComponent, "setAutoDraw"):
            thisComponent.setAutoDraw(False)
    thisExp.addData('Initialization.stopped', globalClock.getTime(format='float'))
    thisExp.nextEntry()
    ...
    for thisTrainingLoopGroup1 in TrainingLoopGroup1:
        ...
        totalTrials1 += 1
        if ResponseKey.keys:  # if a response was made
            response = int(ResponseKey.keys[0])
            if response == int(EnglishCorrectAnswer):  # assuming EnglishCorrectAnswer is the correct answer
                correctResponses1 += 1
        ...
        FeedbackText.setText(feedback_message)
        # Run 'Begin Routine' code from code_14
        feedback_message = f"English: {EnglishCorrectAnswer}\nPersian: {PersianCorrectAnswer}"
        ...
        if totalTrials1 > 0:  # avoid division by zero
