...
expName = '5-15'  # from the Builder filename that created this script
...
def setupData(expInfo, dataDir=None):
    ...
    thisExp = data.ExperimentHandler(
        ...
        originPath='/Users/ohtalab2/Desktop/Experiment Folder/5-15_lastrun.py',
...
def run(expInfo, thisExp, win, globalClock=None, thisSession=None):
    ...
    for thisTrainingLoopGroup1 in TrainingLoopGroup1:
        ...
        # End Routine tab in the Code Component of TrainingGroup1 (similarly for 2 and 3)
        if 'your_response_variable' in your_response_component.keys:
            participant_response_label = your_response_component.keys[-1]  # Get the last key pressed
    ...
    for thisTrainingLoopGroup2 in TrainingLoopGroup2:
        ...
        thisExp.addData('feedbackText', feedbackText)
        # Print for debugging
        print(f"English Correct Answer: {EnglishCorrectAnswer}")
        print(f"Persian Correct Answer: {PersianCorrectAnswer}")
        print(f"Participant Response: {participant_response_label}")
        print(f"Feedback Text: {feedbackText}")
        ...
        # End Routine tab in the Code Component of TrainingGroup1
        if 'your_response_variable' in your_response_component.keys:
            participant_response_label = your_response_component.keys[-1]  # Get the last key pressed
            if participant_response_label == CorrectAnswer:  # Assuming CorrectAnswer is defined
                correct_responses += 1
    ...
    # Initialize total_trials and correct_responses
    total_trials = 0
    correct_responses = 0
    
    # Check if the participant achieved 85% or more correct responses
    if 'TrainingLoopGroup1' in globals():
        total_trials += TrainingLoopGroup1.nTotal
    if 'TrainingLoopGroup2' in globals():
        total_trials += TrainingLoopGroup2.nTotal
    if 'TrainingLoopGroup3' in globals():
        total_trials += TrainingLoopGroup3.nTotal
    
    # Calculate accuracy
    if total_trials > 0:
        accuracy = correctResponses / total_trials
    else:
        accuracy = 0
    ...
    if nextRoutine == 'TestPhase1':
        # Do nothing special, the flow will naturally go to TestPhase1
        pass
    elif nextRoutine == 'TestPhase2':
        # Do nothing special, the flow will naturally go to TestPhase2
        pass
    elif nextRoutine == 'TestPhase3':
        # Do nothing special, the flow will naturally go to TestPhase3
        pass
    else:
    ...
    thisExp.addData('test1_response_keyboard.keys',test1_response_keyboard.keys)
    if test1_response_keyboard.keys != None:  # we had a response
        thisExp.addData('test1_response_keyboard.rt', test1_response_keyboard.rt)
        thisExp.addData('test1_response_keyboard.duration', test1_response_keyboard.duration)
    ...
    thisExp.addData('test2_response_keyboard.keys',test2_response_keyboard.keys)
    if test2_response_keyboard.keys != None:  # we had a response
        thisExp.addData('test2_response_keyboard.rt', test2_response_keyboard.rt)
        thisExp.addData('test2_response_keyboard.duration', test2_response_keyboard.duration)
    ...
    thisExp.addData('test3_response_keyboard.keys',test3_response_keyboard.keys)
    if test3_response_keyboard.keys != None:  # we had a response
        thisExp.addData('test3_response_keyboard.rt', test3_response_keyboard.rt)
        thisExp.addData('test3_response_keyboard.duration', test3_response_keyboard.duration)
