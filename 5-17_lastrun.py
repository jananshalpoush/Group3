...
expName = '5-17'  # from the Builder filename that created this script
...
def setupData(expInfo, dataDir=None):
    ...
    thisExp = data.ExperimentHandler(
        ...
        originPath='/Users/ohtalab2/Desktop/Experiment Folder/5-17_lastrun.py',
...
def setupDevices(expInfo, thisExp, win):
    ...
    if deviceManager.getDevice('Test1_response') is None:
        # initialise Test1_response
        Test1_response = deviceManager.addDevice(
            deviceClass='keyboard',
            deviceName='Test1_response',
        )
    ...
    if deviceManager.getDevice('Test2_response') is None:
        # initialise Test2_response
        Test2_response = deviceManager.addDevice(
            deviceClass='keyboard',
            deviceName='Test2_response',
        )
    ...
    if deviceManager.getDevice('Test3_response') is None:
        # initialise Test3_response
        Test3_response = deviceManager.addDevice(
            deviceClass='keyboard',
            deviceName='Test3_response',
        )
...
def run(expInfo, thisExp, win, globalClock=None, thisSession=None):
    ...
    WelcomeTest1 = visual.TextStim(win=win, name='WelcomeTest1',
        text='',
        font='Arial',
        pos=(0, 0), height=0.05, wrapWidth=None, ori=0.0, 
        color='white', colorSpace='rgb', opacity=None, 
    ...
    text_3 = visual.TextStim(win=win, name='text_3',
        ...
        languageStyle='Arabic',
        depth=-6.0);
    Test1_response = keyboard.Keyboard(deviceName='Test1_response')
    
    # --- Initialize components for Routine "TestPhase2" ---
    WelcomeTest2 = visual.TextStim(win=win, name='WelcomeTest2',
        text='',
        font='Arial',
        pos=(0, 0), height=0.05, wrapWidth=None, ori=0.0, 
        color='white', colorSpace='rgb', opacity=None, 
    ...
    text_8 = visual.TextStim(win=win, name='text_8',
        ...
        languageStyle='Arabic',
        depth=-6.0);
    Test2_response = keyboard.Keyboard(deviceName='Test2_response')
    
    # --- Initialize components for Routine "TestPhase3" ---
    WelcomeTest3 = visual.TextStim(win=win, name='WelcomeTest3',
        text='',
        font='Arial',
        pos=(0, 0), height=0.05, wrapWidth=None, ori=0.0, 
        color='white', colorSpace='rgb', opacity=None, 
    ...
    text_10 = visual.TextStim(win=win, name='text_10',
        ...
        languageStyle='Arabic',
        depth=-6.0);
    Test3_response = keyboard.Keyboard(deviceName='Test3_response')
    ...
    # Run 'Begin Routine' code from check_accuracy
    # Calculate accuracy
    if total_trials > 0:
    ...
    # Log the next routine for debugging
    ...
    # Run 'End Routine' code from check_accuracy
    # Navigate to the next routine
    if nextRoutine == 'TestPhase1':
        TrainingLoopGroup1.finished = True
        TrainingLoopGroup2.finished = True
        TrainingLoopGroup3.finished = True
        continueRoutine = False
    ...
    WelcomeTest1.setText('Weolcome to the Test Phase!\nPlease enter the number corresponding to the speaker you hear.\nبه مرحله ازمون خوش امدید\nلطفا پس از شنیدن جملات٬ شماره گوینده را وارد کنید')
    test1_image1.setImage(SpeakerPhoto1)
    test1_image2.setImage(SpeakerPhoto2)
    test1_image3.setImage(SpeakerPhoto3)
    test1_image4.setImage(SpeakerPhoto4)
    ...
    Test1_response.keys = []
    Test1_response.rt = []
    _Test1_response_allKeys = []
    # keep track of which components have finished
    TestPhase1Components = [WelcomeTest1, test1_image1, test1_image2, test1_image3, test1_image4, test1_sound, text_3, Test1_response]
    ...
    while continueRoutine:
        ...
        # *WelcomeTest1* updates
        
        # if WelcomeTest1 is starting this frame...
        if WelcomeTest1.status == NOT_STARTED and tThisFlip >= 0.0-frameTolerance:
            # keep track of start time/frame for later
            WelcomeTest1.frameNStart = frameN  # exact frame index
            WelcomeTest1.tStart = t  # local t and not account for scr refresh
            WelcomeTest1.tStartRefresh = tThisFlipGlobal  # on global time
            win.timeOnFlip(WelcomeTest1, 'tStartRefresh')  # time at next scr refresh
            # add timestamp to datafile
            thisExp.timestampOnFlip(win, 'WelcomeTest1.started')
            # update status
            WelcomeTest1.status = STARTED
            WelcomeTest1.setAutoDraw(True)
        
        # if WelcomeTest1 is active this frame...
        if WelcomeTest1.status == STARTED:
            # update params
            pass
        
        # if WelcomeTest1 is stopping this frame...
        if WelcomeTest1.status == STARTED:
            # is it time to stop? (based on global clock, using actual start)
            if tThisFlipGlobal > WelcomeTest1.tStartRefresh + 5-frameTolerance:
                # keep track of stop time/frame for later
                WelcomeTest1.tStop = t  # not accounting for scr refresh
                WelcomeTest1.tStopRefresh = tThisFlipGlobal  # on global time
                WelcomeTest1.frameNStop = frameN  # exact frame index
                # add timestamp to datafile
                thisExp.timestampOnFlip(win, 'WelcomeTest1.stopped')
                # update status
                WelcomeTest1.status = FINISHED
                WelcomeTest1.setAutoDraw(False)
        ...
        # *Test1_response* updates
        waitOnFlip = False
        
        # if Test1_response is starting this frame...
        if Test1_response.status == NOT_STARTED and tThisFlip >= 0.0-frameTolerance:
            # keep track of start time/frame for later
            Test1_response.frameNStart = frameN  # exact frame index
            Test1_response.tStart = t  # local t and not account for scr refresh
            Test1_response.tStartRefresh = tThisFlipGlobal  # on global time
            win.timeOnFlip(Test1_response, 'tStartRefresh')  # time at next scr refresh
            # add timestamp to datafile
            thisExp.timestampOnFlip(win, 'Test1_response.started')
            # update status
            Test1_response.status = STARTED
            # keyboard checking is just starting
            waitOnFlip = True
            win.callOnFlip(Test1_response.clock.reset)  # t=0 on next screen flip
            win.callOnFlip(Test1_response.clearEvents, eventType='keyboard')  # clear events on next screen flip
        if Test1_response.status == STARTED and not waitOnFlip:
            theseKeys = Test1_response.getKeys(keyList=['1','2','3','4'], ignoreKeys=["escape"], waitRelease=False)
            _Test1_response_allKeys.extend(theseKeys)
            if len(_Test1_response_allKeys):
                Test1_response.keys = _Test1_response_allKeys[-1].name  # just the last key pressed
                Test1_response.rt = _Test1_response_allKeys[-1].rt
                Test1_response.duration = _Test1_response_allKeys[-1].duration
                # a response ends the routine
                continueRoutine = False
    ...
    # check responses
    if Test1_response.keys in ['', [], None]:  # No response was made
        Test1_response.keys = None
    thisExp.addData('Test1_response.keys',Test1_response.keys)
    if Test1_response.keys != None:  # we had a response
        thisExp.addData('Test1_response.rt', Test1_response.rt)
        thisExp.addData('Test1_response.duration', Test1_response.duration)
    ...
    WelcomeTest2.setText('Weolcome to the Test Phase!\nPlease enter the number corresponding to the speaker you hear.\nبه مرحله ازمون خوش امدید\nلطفا پس از شنیدن جملات٬ شماره گوینده را وارد کنید')
    ...
    Test2_response.keys = []
    Test2_response.rt = []
    _Test2_response_allKeys = []
    # Run 'Begin Routine' code from log_response_test2
    # Print for debugging purposes
    print(f"Starting TestPhase2")
    
    # keep track of which components have finished
    TestPhase2Components = [WelcomeTest2, test2_image1, test2_image2, test2_image3, test2_image4, test2_sound, text_8, Test2_response]
    ...
    while continueRoutine:
        ...
        # *WelcomeTest2* updates
        
        # if WelcomeTest2 is starting this frame...
        if WelcomeTest2.status == NOT_STARTED and tThisFlip >= 0.0-frameTolerance:
            # keep track of start time/frame for later
            WelcomeTest2.frameNStart = frameN  # exact frame index
            WelcomeTest2.tStart = t  # local t and not account for scr refresh
            WelcomeTest2.tStartRefresh = tThisFlipGlobal  # on global time
            win.timeOnFlip(WelcomeTest2, 'tStartRefresh')  # time at next scr refresh
            # add timestamp to datafile
            thisExp.timestampOnFlip(win, 'WelcomeTest2.started')
            # update status
            WelcomeTest2.status = STARTED
            WelcomeTest2.setAutoDraw(True)
        
        # if WelcomeTest2 is active this frame...
        if WelcomeTest2.status == STARTED:
            # update params
            pass
        
        # if WelcomeTest2 is stopping this frame...
        if WelcomeTest2.status == STARTED:
            # is it time to stop? (based on global clock, using actual start)
            if tThisFlipGlobal > WelcomeTest2.tStartRefresh + 5-frameTolerance:
                # keep track of stop time/frame for later
                WelcomeTest2.tStop = t  # not accounting for scr refresh
                WelcomeTest2.tStopRefresh = tThisFlipGlobal  # on global time
                WelcomeTest2.frameNStop = frameN  # exact frame index
                # add timestamp to datafile
                thisExp.timestampOnFlip(win, 'WelcomeTest2.stopped')
                # update status
                WelcomeTest2.status = FINISHED
                WelcomeTest2.setAutoDraw(False)
        ...
        # *Test2_response* updates
        waitOnFlip = False
        
        # if Test2_response is starting this frame...
        if Test2_response.status == NOT_STARTED and tThisFlip >= 0.0-frameTolerance:
            # keep track of start time/frame for later
            Test2_response.frameNStart = frameN  # exact frame index
            Test2_response.tStart = t  # local t and not account for scr refresh
            Test2_response.tStartRefresh = tThisFlipGlobal  # on global time
            win.timeOnFlip(Test2_response, 'tStartRefresh')  # time at next scr refresh
            # add timestamp to datafile
            thisExp.timestampOnFlip(win, 'Test2_response.started')
            # update status
            Test2_response.status = STARTED
            # keyboard checking is just starting
            waitOnFlip = True
            win.callOnFlip(Test2_response.clock.reset)  # t=0 on next screen flip
            win.callOnFlip(Test2_response.clearEvents, eventType='keyboard')  # clear events on next screen flip
        if Test2_response.status == STARTED and not waitOnFlip:
            theseKeys = Test2_response.getKeys(keyList=['1','2','3','4'], ignoreKeys=["escape"], waitRelease=False)
            _Test2_response_allKeys.extend(theseKeys)
            if len(_Test2_response_allKeys):
                Test2_response.keys = _Test2_response_allKeys[-1].name  # just the last key pressed
                Test2_response.rt = _Test2_response_allKeys[-1].rt
                Test2_response.duration = _Test2_response_allKeys[-1].duration
                # a response ends the routine
                continueRoutine = False
    ...
    # check responses
    if Test2_response.keys in ['', [], None]:  # No response was made
        Test2_response.keys = None
    thisExp.addData('Test2_response.keys',Test2_response.keys)
    if Test2_response.keys != None:  # we had a response
        thisExp.addData('Test2_response.rt', Test2_response.rt)
        thisExp.addData('Test2_response.duration', Test2_response.duration)
    # Run 'End Routine' code from log_response_test2
    # Record participant's response
    if 'Test2_Response' in globals() and 'keys' in Test2_Response and len(Test2_Response.keys) > 0:
        response = Test2_Response.keys[-1]  # Get the last key pressed
        thisExp.addData('Test2_Response', response)
    else:
    ...
    WelcomeTest3.setText('Weolcome to the Test Phase!\nPlease enter the number corresponding to the speaker you hear.\nبه مرحله ازمون خوش امدید\nلطفا پس از شنیدن جملات٬ شماره گوینده را وارد کنید')
    ...
    Test3_response.keys = []
    Test3_response.rt = []
    _Test3_response_allKeys = []
    # Run 'Begin Routine' code from log_response_test3
    # Print for debugging purposes
    print(f"Starting TestPhase3")
    
    # keep track of which components have finished
    TestPhase3Components = [WelcomeTest3, test3_image1, test3_image2, test3_image3, test3_image4, test3_sound, text_10, Test3_response]
    ...
    while continueRoutine:
        ...
        # *WelcomeTest3* updates
        
        # if WelcomeTest3 is starting this frame...
        if WelcomeTest3.status == NOT_STARTED and tThisFlip >= 0.0-frameTolerance:
            # keep track of start time/frame for later
            WelcomeTest3.frameNStart = frameN  # exact frame index
            WelcomeTest3.tStart = t  # local t and not account for scr refresh
            WelcomeTest3.tStartRefresh = tThisFlipGlobal  # on global time
            win.timeOnFlip(WelcomeTest3, 'tStartRefresh')  # time at next scr refresh
            # add timestamp to datafile
            thisExp.timestampOnFlip(win, 'WelcomeTest3.started')
            # update status
            WelcomeTest3.status = STARTED
            WelcomeTest3.setAutoDraw(True)
        
        # if WelcomeTest3 is active this frame...
        if WelcomeTest3.status == STARTED:
            # update params
            pass
        
        # if WelcomeTest3 is stopping this frame...
        if WelcomeTest3.status == STARTED:
            # is it time to stop? (based on global clock, using actual start)
            if tThisFlipGlobal > WelcomeTest3.tStartRefresh + 5-frameTolerance:
                # keep track of stop time/frame for later
                WelcomeTest3.tStop = t  # not accounting for scr refresh
                WelcomeTest3.tStopRefresh = tThisFlipGlobal  # on global time
                WelcomeTest3.frameNStop = frameN  # exact frame index
                # add timestamp to datafile
                thisExp.timestampOnFlip(win, 'WelcomeTest3.stopped')
                # update status
                WelcomeTest3.status = FINISHED
                WelcomeTest3.setAutoDraw(False)
        ...
        # *Test3_response* updates
        waitOnFlip = False
        
        # if Test3_response is starting this frame...
        if Test3_response.status == NOT_STARTED and tThisFlip >= 0.0-frameTolerance:
            # keep track of start time/frame for later
            Test3_response.frameNStart = frameN  # exact frame index
            Test3_response.tStart = t  # local t and not account for scr refresh
            Test3_response.tStartRefresh = tThisFlipGlobal  # on global time
            win.timeOnFlip(Test3_response, 'tStartRefresh')  # time at next scr refresh
            # add timestamp to datafile
            thisExp.timestampOnFlip(win, 'Test3_response.started')
            # update status
            Test3_response.status = STARTED
            # keyboard checking is just starting
            waitOnFlip = True
            win.callOnFlip(Test3_response.clock.reset)  # t=0 on next screen flip
            win.callOnFlip(Test3_response.clearEvents, eventType='keyboard')  # clear events on next screen flip
        if Test3_response.status == STARTED and not waitOnFlip:
            theseKeys = Test3_response.getKeys(keyList=['1','2','3','4'], ignoreKeys=["escape"], waitRelease=False)
            _Test3_response_allKeys.extend(theseKeys)
            if len(_Test3_response_allKeys):
                Test3_response.keys = _Test3_response_allKeys[-1].name  # just the last key pressed
                Test3_response.rt = _Test3_response_allKeys[-1].rt
                Test3_response.duration = _Test3_response_allKeys[-1].duration
                # a response ends the routine
                continueRoutine = False
    ...
    # check responses
    if Test3_response.keys in ['', [], None]:  # No response was made
        Test3_response.keys = None
    thisExp.addData('Test3_response.keys',Test3_response.keys)
    if Test3_response.keys != None:  # we had a response
        thisExp.addData('Test3_response.rt', Test3_response.rt)
        thisExp.addData('Test3_response.duration', Test3_response.duration)
    # Run 'End Routine' code from log_response_test3
    # Record participant's response
    if 'Test3_Response' in globals() and 'keys' in Test3_Response and len(Test3_Response.keys) > 0:
        response = Test3_Response.keys[-1]  # Get the last key pressed
        thisExp.addData('Test3_Response', response)
    else:
