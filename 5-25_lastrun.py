...
expName = '5-25'  # from the Builder filename that created this script
...
def setupData(expInfo, dataDir=None):
    ...
    thisExp = data.ExperimentHandler(
        ...
        originPath='/Users/ohtalab2/Desktop/Experiment Folder/5-25_lastrun.py',
...
def run(expInfo, thisExp, win, globalClock=None, thisSession=None):
    ...
    TrainingLoopGroup1 = data.TrialHandler(nReps=1000.0, method='random', 
    ...
    for thisTrainingLoopGroup1 in TrainingLoopGroup1:
        ...
        accuracy_group1 = correct_responses_group1 / total_trials_group1 if total_trials_group1 > 0 else 0
        
        # Check if the participant met the accuracy criterion
        proceed_to_test = accuracy_group1 >= TARGET_ACCURACY
        
        # Log data for debugging (optional)
        thisExp.addData('accuracy_group1', accuracy_group1)
        thisExp.addData('proceed_to_test_group1', proceed_to_test)
    ...
    # completed 1000.0 repeats of 'TrainingLoopGroup1'
    
    
    # set up handler to look after randomisation of conditions etc
    TrainingLoopGroup2 = data.TrialHandler(nReps=1000.0, method='random', 
        extraInfo=expInfo, originPath=-1,
    ...
    for thisTrainingLoopGroup2 in TrainingLoopGroup2:
        ...
        accuracy_group2 = correct_responses_group2 / total_trials_group2 if total_trials_group2 > 0 else 0
        
        # Check if the participant met the accuracy criterion
        proceed_to_test = accuracy_group2 >= TARGET_ACCURACY
        
        # Log data for debugging (optional)
        thisExp.addData('accuracy_group2', accuracy_group2)
        thisExp.addData('proceed_to_test_group2', proceed_to_test)
        ...
        if thisSession is not None:
            # if running in a Session with a Liaison client, send data up to now
            thisSession.sendExperimentData()
    # completed 1000.0 repeats of 'TrainingLoopGroup2'
    
    
    # set up handler to look after randomisation of conditions etc
    TrainingLoopGroup3 = data.TrialHandler(nReps=50.0, method='random', 
        extraInfo=expInfo, originPath=-1,
    ...
    for thisTrainingLoopGroup3 in TrainingLoopGroup3:
        ...
        if len(Response3.keys) > 0:  # Check if a key was pressed
            ...
            total_trials_group3 += 1  # Increment total trials after a response is recorded
        ...
        # Calculate accuracy for group 3
        accuracy_group3 = correct_responses_group3 / total_trials_group3 if total_trials_group3 > 0 else 0
        
        # Check if the participant met the accuracy criterion
        proceed_to_test = accuracy_group3 >= TARGET_ACCURACY
        
        # Log data for debugging (optional)
        thisExp.addData('accuracy_group3', accuracy_group3)
        thisExp.addData('proceed_to_test_group3', proceed_to_test)
        ...
        if thisSession is not None:
            # if running in a Session with a Liaison client, send data up to now
            thisSession.sendExperimentData()
    # completed 50.0 repeats of 'TrainingLoopGroup3'
