...
expName = '5-26'  # from the Builder filename that created this script
...
def setupData(expInfo, dataDir=None):
    ...
    thisExp = data.ExperimentHandler(
        ...
        originPath='/Users/ohtalab2/Desktop/Experiment Folder/5-26_lastrun.py',
...
def run(expInfo, thisExp, win, globalClock=None, thisSession=None):
    ...
    # Run 'Begin Experiment' code from var
    # Initialize score variables for each group
    score_group3 = 0
    ...
    # Initialize score variables for Training Group 3
    score_group3 = 0
    total_trials_group3 = 0
    TARGET_ACCURACY = 0.85
    ...
    # --- Initialize components for Routine "test_group3" ---
    ...
    for thisTrainingLoopGroup1 in TrainingLoopGroup1:
        ...
        if 'ResponseKey' in globals() and 'keys' in ResponseKey and len(ResponseKey.keys) > 0:
            ...
            if participant_response_label == CorrectAnswer:  # Assuming CorrectAnswer is defined
                correct_responses_group1 += 1
            total_trials_group1 += 1
        
        # Log data for debugging (optional)
        thisExp.addData('correct_responses_group1', correct_responses_group1)
        thisExp.addData('total_trials_group1', total_trials_group1)
        
        # Calculate accuracy after updating counters
        accuracy_group1 = correct_responses_group1 / total_trials_group1 if total_trials_group1 > 0 else 0.0
        
        # Check if participant meets the accuracy criterion for Group 1
        proceed_to_test = accuracy_group1 >= TARGET_ACCURACY
        
        # If accuracy criterion not met, set flag to retry training
        retry_training = not proceed_to_test
    ...
    for thisTrainingLoopGroup2 in TrainingLoopGroup2:
        ...
        if len(Response2.keys) > 0:  # Check if a key was pressed
            ...
            if participant_response_label == CorrectAnswer:  # Assuming CorrectAnswer is defined
                correct_responses_group2 += 1
            total_trials_group2 += 1
        
        # Log data for debugging (optional)
        thisExp.addData('correct_responses_group2', correct_responses_group2)
        thisExp.addData('total_trials_group2', total_trials_group2)
        
        # Calculate accuracy after updating counters
        accuracy_group2 = correct_responses_group2 / total_trials_group2 if total_trials_group2 > 0 else 0.0
        
        # Check if participant meets the accuracy criterion for Group 2
        proceed_to_test = accuracy_group2 >= TARGET_ACCURACY
    ...
    # set up handler to look after randomisation of conditions etc
    Repeat_TrainingLoopGroup3 = data.TrialHandler(nReps=100.0, method='random', 
        extraInfo=expInfo, originPath=-1,
        trialList=data.importConditions('English Trained- Training Phase.xlsx'),
        seed=None, name='Repeat_TrainingLoopGroup3')
    thisExp.addLoop(Repeat_TrainingLoopGroup3)  # add the loop to the experiment
    thisRepeat_TrainingLoopGroup3 = Repeat_TrainingLoopGroup3.trialList[0]  # so we can initialise stimuli with some values
    # abbreviate parameter names if possible (e.g. rgb = thisRepeat_TrainingLoopGroup3.rgb)
    if thisRepeat_TrainingLoopGroup3 != None:
        for paramName in thisRepeat_TrainingLoopGroup3:
            globals()[paramName] = thisRepeat_TrainingLoopGroup3[paramName]
    
    for thisRepeat_TrainingLoopGroup3 in Repeat_TrainingLoopGroup3:
        currentLoop = Repeat_TrainingLoopGroup3
        thisExp.timestampOnFlip(win, 'thisRow.t', format=globalClock.format)
        # pause experiment here if requested
        if thisExp.status == PAUSED:
            pauseExperiment(
        ...
        for thisTrainingLoopGroup3 in TrainingLoopGroup3:
            ...
            if TrainingGroup == 3:
                if score_group3 / total_trials_group3 > 0.85:
            ...
            if len(Response3.keys) > 0:  # Check if a key was pressed
                ...
                if participant_response_label == CorrectAnswer:
                    ...
                    feedbackText = f"Incorrect! The correct answer is {EnglishCorrectAnswer}. نادرست! پاسخ صحیح {PersianCorrectAnswer} است"
                total_trials_group3 += 1  # Update total trials
            ...
            # Add this data for debugging
            thisExp.addData('correct_responses_group3', correct_responses_group3)
            thisExp.addData('total_trials_group3', total_trials_group3)
            ...
            if len(Response3.keys) > 0:  # Check if a key was pressed
                ...
                if participant_response_label == CorrectAnswer:  # Assuming CorrectAnswer is defined
                    correct_responses_group3 += 1
                total_trials_group3 += 1
            ...
            # Calculate accuracy after updating counters
            accuracy_group3 = 68 / 80 if 80 > 0 else 0.0
            
            # Check if participant meets the accuracy criterion for Group 1
            proceed_to_test_group3 = accuracy_group3 >= 0.85
        ...
        # Check accuracy for Training Group 3
        if total_trials_group3 > 0:
            accuracy_group3 = score_group3 / total_trials_group3
        else:
            accuracy_group3 = 0
        
        # Determine whether to proceed or repeat training
        if accuracy_group3 >= TARGET_ACCURACY:
            repeated_TrainingLoopGroup3.finished = True
        else:
            score_group3 = 0  # Reset the score for the next training attempt
            total_trials_group3 = 0  # Reset total trials for the next training attempt
        ...
        # Update the score and total trials based on the correctness of the response
        if Response3:  # Assuming 'trainingresp' is the name of your response object
            score_group3 += 1
        total_trials_group3 += 1
        ...
        if thisSession is not None:
            # if running in a Session with a Liaison client, send data up to now
            thisSession.sendExperimentData()
    # completed 100.0 repeats of 'Repeat_TrainingLoopGroup3'
    ...
    for thisTestPhase3Loop in testPhase3Loop:
        ...
        # --- Prepare to start Routine "test_group3" ---
        continueRoutine = True
        # update component parameters for each repeat
        thisExp.addData('test_group3.started', globalClock.getTime(format='float'))
        ...
        # keep track of which components have finished
        test_group3Components = [test3_image1, test3_image2, test3_image3, test3_image4, test3_sound, text_10, test3_response_keyboard]
        for thisComponent in test_group3Components:
            thisComponent.tStart = None
            thisComponent.tStop = None
            thisComponent.tStartRefresh = None
            thisComponent.tStopRefresh = None
        ...
        # --- Ending Routine "test_group3" ---
        for thisComponent in test_group3Components:
            if hasattr(thisComponent, "setAutoDraw"):
                thisComponent.setAutoDraw(False)
        thisExp.addData('test_group3.stopped', globalClock.getTime(format='float'))
        ...
        # the Routine "test_group3" was not non-slip safe, so reset the non-slip timer
        routineTimer.reset()
        thisExp.nextEntry()
