...
expName = '5-9'  # from the Builder filename that created this script
...
def setupData(expInfo, dataDir=None):
    ...
    thisExp = data.ExperimentHandler(
        ...
        originPath='/Users/ohtalab2/Desktop/Experiment Folder/5-9_lastrun.py',
...
def run(expInfo, thisExp, win, globalClock=None, thisSession=None):
    ...
    # Run 'Begin Experiment' code from Initialize_code
    # Initialize counters for training groups
    totalTrials1 = 0
    correctResponses1 = 0
    totalTrials2 = 0
    correctResponses2 = 0
    ...
    # --- Initialize components for Routine "PerformanceCheck1" ---
    ...
    # --- Initialize components for Routine "PerformaceCheck2" ---
    ...
    # --- Initialize components for Routine "PerformanceCheck3" ---
    ...
    # --- Prepare to start Routine "PerformanceCheck1" ---
    continueRoutine = True
    # update component parameters for each repeat
    thisExp.addData('PerformanceCheck1.started', globalClock.getTime(format='float'))
    # Run 'Begin Routine' code from code_3
    # Calculate accuracy for TrainingGroup1
    accuracy1 = correctResponses1 / totalTrials1
    
    # Check if the accuracy is above 85%
    if accuracy1 >= 0.85:
        passGroup1 = True
    else:
        passGroup1 = False
    
    # Save accuracy data for TrainingGroup1
    thisExp.addData('accuracy1', accuracy1)
    thisExp.addData('passGroup1', passGroup1)
    # keep track of which components have finished
    PerformanceCheck1Components = []
    for thisComponent in PerformanceCheck1Components:
        thisComponent.tStart = None
        thisComponent.tStop = None
        thisComponent.tStartRefresh = None
        thisComponent.tStopRefresh = None
    ...
    # --- Ending Routine "PerformanceCheck1" ---
    for thisComponent in PerformanceCheck1Components:
        if hasattr(thisComponent, "setAutoDraw"):
            thisComponent.setAutoDraw(False)
    thisExp.addData('PerformanceCheck1.stopped', globalClock.getTime(format='float'))
    # Run 'End Routine' code from code_3
    # Decide whether to proceed to the test phase or repeat the training phase
    if passGroup1:
        continueRoutine = False  # Move to the testing phase
    else:
        continueRoutine = True   # Repeat the training phase
    
    # Reset counters for the next attempt if repeating the training phase
    if continueRoutine:
        totalTrials1 = 0
        correctResponses1 = 0
    thisExp.nextEntry()
    # the Routine "PerformanceCheck1" was not non-slip safe, so reset the non-slip timer
    routineTimer.reset()
    
    # set up handler to look after randomisation of conditions etc
    TrainingLoopGroup2 = data.TrialHandler(nReps=999.0, method='random', 
        extraInfo=expInfo, originPath=-1,
    ...
    for thisTrainingLoopGroup2 in TrainingLoopGroup2:
        ...
        # Initialize variables if not already done
        if 'correctResponses2' not in globals():
            correctResponses2 = 0
        
        # Mapping numeric responses to speaker labels (assuming it's the same for TrainingGroup1)
        response_mapping = {
            '1': 'Speaker1',
            '2': 'Speaker2',
            '3': 'Speaker3',
        ...
        # Ensure CorrectAnswer is correctly defined (assuming it's the same as TrainingGroup1)
        CorrectAnswer = EnglishCorrectAnswer  # Using EnglishCorrectAnswer for comparison
        
        # Map participant response to speaker label (assuming it's the same as TrainingGroup1)
        participant_response_label = response_mapping.get(ResponseKey.keys[0], "Invalid")
        ...
        if thisSession is not None:
            # if running in a Session with a Liaison client, send data up to now
            thisSession.sendExperimentData()
    # completed 999.0 repeats of 'TrainingLoopGroup2'
    
    
    # --- Prepare to start Routine "PerformaceCheck2" ---
    continueRoutine = True
    # update component parameters for each repeat
    thisExp.addData('PerformaceCheck2.started', globalClock.getTime(format='float'))
    # Run 'Begin Routine' code from code_8
    # Calculate accuracy for TrainingGroup2
    accuracy2 = correctResponses2 / totalTrials2
    ...
    # Save accuracy data for TrainingGroup2
    thisExp.addData('accuracy2', accuracy2)
    thisExp.addData('passGroup2', passGroup2)
    # keep track of which components have finished
    PerformaceCheck2Components = []
    for thisComponent in PerformaceCheck2Components:
        thisComponent.tStart = None
        thisComponent.tStop = None
        thisComponent.tStartRefresh = None
        thisComponent.tStopRefresh = None
    ...
    # --- Ending Routine "PerformaceCheck2" ---
    for thisComponent in PerformaceCheck2Components:
        if hasattr(thisComponent, "setAutoDraw"):
            thisComponent.setAutoDraw(False)
    thisExp.addData('PerformaceCheck2.stopped', globalClock.getTime(format='float'))
    ...
    if continueRoutine:
        ...
        correctResponses2 = 0
    thisExp.nextEntry()
    # the Routine "PerformaceCheck2" was not non-slip safe, so reset the non-slip timer
    routineTimer.reset()
    
    # set up handler to look after randomisation of conditions etc
    TrainingLoopGroup3 = data.TrialHandler(nReps=999.0, method='random', 
        extraInfo=expInfo, originPath=-1,
    ...
    for thisTrainingLoopGroup3 in TrainingLoopGroup3:
        ...
        # Initialize variables if not already done
        if 'correctResponses3' not in globals():
            correctResponses3 = 0
        ...
        if thisSession is not None:
            # if running in a Session with a Liaison client, send data up to now
            thisSession.sendExperimentData()
    # completed 999.0 repeats of 'TrainingLoopGroup3'
    
    
    # --- Prepare to start Routine "PerformanceCheck3" ---
    continueRoutine = True
    # update component parameters for each repeat
    thisExp.addData('PerformanceCheck3.started', globalClock.getTime(format='float'))
    # Run 'Begin Routine' code from code_6
    # Calculate accuracy for TrainingGroup3
    accuracy3 = correctResponses3 / totalTrials3
    ...
    # Save accuracy data for TrainingGroup3
    thisExp.addData('accuracy3', accuracy3)
    thisExp.addData('passGroup3', passGroup3)
    # keep track of which components have finished
    PerformanceCheck3Components = []
    for thisComponent in PerformanceCheck3Components:
        thisComponent.tStart = None
        thisComponent.tStop = None
        thisComponent.tStartRefresh = None
        thisComponent.tStopRefresh = None
    ...
    # --- Ending Routine "PerformanceCheck3" ---
    for thisComponent in PerformanceCheck3Components:
        if hasattr(thisComponent, "setAutoDraw"):
            thisComponent.setAutoDraw(False)
    thisExp.addData('PerformanceCheck3.stopped', globalClock.getTime(format='float'))
    ...
    if continueRoutine:
        ...
        correctResponses3 = 0
    thisExp.nextEntry()
    # the Routine "PerformanceCheck3" was not non-slip safe, so reset the non-slip timer
    routineTimer.reset()
