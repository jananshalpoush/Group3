...
expName = '5-32 ready-Group3 English Trained'  # from the Builder filename that created this script
...
def setupData(expInfo, dataDir=None):
    ...
    thisExp = data.ExperimentHandler(
        ...
        originPath='/Users/ohtalab2/Desktop/Experiment Folder/Experiment3/Group3_lastrun.py',
...
def run(expInfo, thisExp, win, globalClock=None, thisSession=None):
    ...
    text_14 = visual.TextStim(win=win, name='text_14',
        text='In the Training Phase, you will receive feedback.\n\nIn the Test Phase, there will be no feedback.',
        font='Arial',
        pos=(0, 0), height=0.05, wrapWidth=None, ori=0.0, 
        color='white', colorSpace='rgb', opacity=None, 
        languageStyle='LTR',
    ...
    TrainingGroup = 3  # or 2, or 3 depending on your experiment setup
    ...
    break_message = visual.TextStim(win=win, name='break_message',
        text="Please take a break, or click the box below to continue when you're ready!",
        font='Arial',
        pos=(0, 0), height=0.05, wrapWidth=None, ori=0.0, 
        color='white', colorSpace='rgb', opacity=None, 
        languageStyle='LTR',
    ...
    checkbox3 = visual.Rect(
        win=win, name='checkbox3',
        width=(0.1, 0.1)[0], height=(0.1, 0.1)[1],
        ori=0.0, pos=(0, -0.2), anchor='center',
        lineWidth=1.0,     colorSpace='rgb',  lineColor='white', fillColor='white',
    ...
    while continueRoutine and routineTimer.getTime() < 5.0:
        ...
        if key_resp.status == STARTED and not waitOnFlip:
            theseKeys = key_resp.getKeys(keyList=['3'], ignoreKeys=["escape"], waitRelease=False)
    ...
    # keep track of which components have finished
    IntroductionComponents = [text_2, image, sound_7, text_4, image_7, sound_9, text_5, image_8, sound_11, text_6, image_9, sound_13, text_7]
    ...
    while continueRoutine and routineTimer.getTime() < 36.0:
        ...
        if sound_7.status == NOT_STARTED and tThisFlip >= 7-frameTolerance:
            # keep track of start time/frame for later
        ...
        if sound_11.status == NOT_STARTED and tThisFlip >= 23-frameTolerance:
            # keep track of start time/frame for later
        ...
        if sound_13.status == NOT_STARTED and tThisFlip >= 31-frameTolerance:
            # keep track of start time/frame for later
    ...
    # keep track of which components have finished
    InstructionsComponents = [InstructionText, text_14]
    ...
    while continueRoutine and routineTimer.getTime() < 20.0:
        ...
        if text_14.status == NOT_STARTED and tThisFlip >= 10-frameTolerance:
            # keep track of start time/frame for later
        ...
        if text_14.status == STARTED:
            # is it time to stop? (based on global clock, using actual start)
            if tThisFlipGlobal > text_14.tStartRefresh + 10-frameTolerance:
                # keep track of stop time/frame for later
    ...
    repeated_TrainingLoopGroup3 = data.TrialHandler(nReps=999.0, method='random', 
        extraInfo=expInfo, originPath=-1,
        trialList=data.importConditions('English Trained- Training Phase.xlsx'),
        seed=None, name='repeated_TrainingLoopGroup3')
    thisExp.addLoop(repeated_TrainingLoopGroup3)  # add the loop to the experiment
    thisRepeated_TrainingLoopGroup3 = repeated_TrainingLoopGroup3.trialList[0]  # so we can initialise stimuli with some values
    # abbreviate parameter names if possible (e.g. rgb = thisRepeated_TrainingLoopGroup3.rgb)
    if thisRepeated_TrainingLoopGroup3 != None:
        for paramName in thisRepeated_TrainingLoopGroup3:
            globals()[paramName] = thisRepeated_TrainingLoopGroup3[paramName]
    
    for thisRepeated_TrainingLoopGroup3 in repeated_TrainingLoopGroup3:
        currentLoop = repeated_TrainingLoopGroup3
        ...
        # set up handler to look after randomisation of conditions etc
        TrainingLoopGroup3 = data.TrialHandler(nReps=1.0, method='random', 
            extraInfo=expInfo, originPath=-1,
            trialList=data.importConditions('English Trained- Training Phase.xlsx'),
        ...
        for thisTrainingLoopGroup3 in TrainingLoopGroup3:
            ...
            # create starting attributes for Response3
            Response3.keys = []
            Response3.rt = []
            _Response3_allKeys = []
            ...
            # keep track of which components have finished
            TrainingGroup3Components = [SpeakerPicture3, image_15, image_16, image_17, Sound3, Instruction3, Response3, SessionNumberText]
            ...
            if len(Response3.keys) > 0:  # Check if a key was pressed
                ...
                if participant_response_label == CorrectAnswer:
                    ...
                    feedbackText = f"Incorrect! The correct answer is {EnglishCorrectAnswer}."
            ...
            thisExp.addData('correct_responses_group3', correctResponses)
            thisExp.addData('total_trials_group3', total_trials)
            ...
            if TrainingLoopGroup3.thisN != 39:
                continueRoutine = False
            ...
            # keep track of which components have finished
            BreakComponents = [break_message, mouse3, checkbox3, text_component]
            ...
            while continueRoutine:
                ...
                if mouse3.status == STARTED:  # only update if started and not finished!
                    ...
                    if buttons != prevButtonState:  # button state changed?
                        ...
                        if sum(buttons) > 0:  # state changed to a new click
                            # check if the mouse was inside our 'clickable' objects
                            gotValidClick = False
                            clickableList = environmenttools.getFromNames(checkbox3, namespace=locals())
                            for obj in clickableList:
                                # is this object clicked on?
                ...
                # if checkbox3 is starting this frame...
                if checkbox3.status == NOT_STARTED and tThisFlip >= 0.0-frameTolerance:
                    # keep track of start time/frame for later
                    checkbox3.frameNStart = frameN  # exact frame index
                    checkbox3.tStart = t  # local t and not account for scr refresh
                    checkbox3.tStartRefresh = tThisFlipGlobal  # on global time
                    win.timeOnFlip(checkbox3, 'tStartRefresh')  # time at next scr refresh
                    # add timestamp to datafile
                    thisExp.timestampOnFlip(win, 'checkbox3.started')
                    # update status
                    checkbox3.status = STARTED
                    checkbox3.setAutoDraw(True)
                
                # if checkbox3 is active this frame...
                if checkbox3.status == STARTED:
                    # update params
                    pass
                # Run 'Each Frame' code from code_17
                if mouse.isPressedIn(checkbox3):  # Use the actual name of the polygon component
                    checkbox_ticked = not checkbox_ticked
                    if checkbox_ticked:
                        checkbox.fillColor = 'black'  # Change color to indicate it's ticked
                    else:
                ...
                text_component.setText(f'Remaining time: {remaining_time:.1f} seconds')
                text_component.setFont('Arial')  # Set to a font that supports English
            ...
            # store data for TrainingLoopGroup3 (TrialHandler)
            TrainingLoopGroup3.addData('mouse3.x', mouse3.x)
            TrainingLoopGroup3.addData('mouse3.y', mouse3.y)
            TrainingLoopGroup3.addData('mouse3.leftButton', mouse3.leftButton)
            TrainingLoopGroup3.addData('mouse3.midButton', mouse3.midButton)
            TrainingLoopGroup3.addData('mouse3.rightButton', mouse3.rightButton)
            TrainingLoopGroup3.addData('mouse3.time', mouse3.time)
            TrainingLoopGroup3.addData('mouse3.clicked_name', mouse3.clicked_name)
        ...
        # completed 1.0 repeats of 'TrainingLoopGroup3'
        
        
        # --- Prepare to start Routine "CheckAccuracy3" ---
        continueRoutine = True
        # update component parameters for each repeat
        thisExp.addData('CheckAccuracy3.started', globalClock.getTime(format='float'))
        ...
        # Log data for debugging (optional)
        thisExp.addData('accuracy_group3', accuracy)
        thisExp.addData('proceed_to_test', proceed_to_test)
        
        # keep track of which components have finished
        CheckAccuracy3Components = []
        for thisComponent in CheckAccuracy3Components:
            thisComponent.tStart = None
            thisComponent.tStop = None
            thisComponent.tStartRefresh = None
            thisComponent.tStopRefresh = None
        ...
        for thisComponent in CheckAccuracy3Components:
            if hasattr(thisComponent, "setAutoDraw"):
                thisComponent.setAutoDraw(False)
        thisExp.addData('CheckAccuracy3.stopped', globalClock.getTime(format='float'))
        ...
        if thisSession is not None:
            # if running in a Session with a Liaison client, send data up to now
            thisSession.sendExperimentData()
    # completed 999.0 repeats of 'repeated_TrainingLoopGroup3'
    ...
    for thisTestPhase3Loop in testPhase3Loop:
        ...
        # create starting attributes for test3_response_keyboard
