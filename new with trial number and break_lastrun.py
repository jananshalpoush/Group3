...
def setupData(expInfo, dataDir=None):
    ...
    thisExp = data.ExperimentHandler(
        ...
        originPath='/Users/ohtalab2/Desktop/Experiment Folder/new with trial number and break_lastrun.py',
...
def run(expInfo, thisExp, win, globalClock=None, thisSession=None):
    ...
    sound_8.setSound('Audio/Speaker1_Per/P1-23-Per.wav', secs=8, hamming=True)
    sound_8.setVolume(1.0, log=False)
    sound_8.seek(0)
    text_4.setText('گوینده ۱')
    image_7.setImage('Images/Speaker2.jpg')
    sound_10.setSound('Audio/Speaker2_Per/P2-23-Per.wav', secs=8, hamming=True)
    sound_10.setVolume(1.0, log=False)
    sound_10.seek(0)
    text_5.setText('گوینده ۲\n')
    image_8.setImage('Images/Speaker3.jpg')
    sound_12.setSound('Audio/Speaker3_Per/P3-23-Per.wav', secs=8, hamming=True)
    sound_12.setVolume(1.0, log=False)
    sound_12.seek(0)
    text_6.setText('گوینده ۳')
    image_9.setImage('Images/Speaker4.jpg')
    sound_14.setSound('Audio/Speaker4_Per/P4-23-Per.wav', secs=8, hamming=True)
    ...
    routineForceEnded = not continueRoutine
    while continueRoutine and routineTimer.getTime() < 37.0:
        # get current time
        t = routineTimer.getTime()
        tThisFlip = win.getFutureFlipTime(clock=routineTimer)
        tThisFlipGlobal = win.getFutureFlipTime(clock=None)
        ...
        if sound_8.status == STARTED:
            # is it time to stop? (based on global clock, using actual start)
            if tThisFlipGlobal > sound_8.tStartRefresh + 8-frameTolerance:
                # keep track of stop time/frame for later
        ...
        if image_7.status == NOT_STARTED and tThisFlip >= 13-frameTolerance:
            # keep track of start time/frame for later
        ...
        if sound_10.status == NOT_STARTED and tThisFlip >= 13-frameTolerance:
            # keep track of start time/frame for later
        ...
        if sound_10.status == STARTED:
            # is it time to stop? (based on global clock, using actual start)
            if tThisFlipGlobal > sound_10.tStartRefresh + 8-frameTolerance:
                # keep track of stop time/frame for later
        ...
        if image_8.status == NOT_STARTED and tThisFlip >= 21-frameTolerance:
            # keep track of start time/frame for later
        ...
        if image_8.status == STARTED:
            # is it time to stop? (based on global clock, using actual start)
            if tThisFlipGlobal > image_8.tStartRefresh + 8-frameTolerance:
                # keep track of stop time/frame for later
        ...
        if sound_12.status == NOT_STARTED and tThisFlip >= 21-frameTolerance:
            # keep track of start time/frame for later
        ...
        if sound_12.status == STARTED:
            # is it time to stop? (based on global clock, using actual start)
            if tThisFlipGlobal > sound_12.tStartRefresh + 8-frameTolerance:
                # keep track of stop time/frame for later
        ...
        if text_6.status == NOT_STARTED and tThisFlip >= 21-frameTolerance:
            # keep track of start time/frame for later
        ...
        if image_9.status == NOT_STARTED and tThisFlip >= 29-frameTolerance:
            # keep track of start time/frame for later
        ...
        if image_9.status == STARTED:
            # is it time to stop? (based on global clock, using actual start)
            if tThisFlipGlobal > image_9.tStartRefresh + 8-frameTolerance:
                # keep track of stop time/frame for later
        ...
        if sound_14.status == NOT_STARTED and tThisFlip >= 29-frameTolerance:
            # keep track of start time/frame for later
        ...
        if sound_14.status == STARTED:
            # is it time to stop? (based on global clock, using actual start)
            if tThisFlipGlobal > sound_14.tStartRefresh + 8-frameTolerance:
                # keep track of stop time/frame for later
        ...
        if text_7.status == NOT_STARTED and tThisFlip >= 29-frameTolerance:
            # keep track of start time/frame for later
    ...
    if routineForceEnded:
        ...
        routineTimer.addTime(-37.000000)
    thisExp.nextEntry()
    ...
    for thisRepeated_TrainingLoopGroup1 in repeated_TrainingLoopGroup1:
        ...
        for thisTrainingLoopGroup1 in TrainingLoopGroup1:
            ...
            while continueRoutine:
                ...
                # if SessionNumberText is stopping this frame...
                if SessionNumberText.status == STARTED:
                    # is it time to stop? (based on global clock, using actual start)
                    if tThisFlipGlobal > SessionNumberText.tStartRefresh + 25-frameTolerance:
                        # keep track of stop time/frame for later
                        SessionNumberText.tStop = t  # not accounting for scr refresh
                        SessionNumberText.tStopRefresh = tThisFlipGlobal  # on global time
                        SessionNumberText.frameNStop = frameN  # exact frame index
                        # add timestamp to datafile
                        thisExp.timestampOnFlip(win, 'SessionNumberText.stopped')
                        # update status
                        SessionNumberText.status = FINISHED
                        SessionNumberText.setAutoDraw(False)
            ...
            # keep track of which components have finished
            BreakComponents = [break_message, mouse1, checkbox1]
            ...
            while continueRoutine:
                ...
                # if break_message is stopping this frame...
                if break_message.status == STARTED:
                    # is it time to stop? (based on global clock, using actual start)
                    if tThisFlipGlobal > break_message.tStartRefresh + 10-frameTolerance:
                        # keep track of stop time/frame for later
                        break_message.tStop = t  # not accounting for scr refresh
                        break_message.tStopRefresh = tThisFlipGlobal  # on global time
                        break_message.frameNStop = frameN  # exact frame index
                        # add timestamp to datafile
                        thisExp.timestampOnFlip(win, 'break_message.stopped')
                        # update status
                        break_message.status = FINISHED
